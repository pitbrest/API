...
class RandomPlanet extends Component {
  ...
  getId = async () => {
    await this.myApi.getPlanets()
      .then(data => {
        this.setState({
          id: this.getRandomIntInclusive(2, data.results.length),
        });
      });
  };
  ...
  updatePlanet = async () => {
    await this.getId();
    this.myApi.getPlanet(this.state.id)
      .then(planet => {
        this.setState({
      ...
      <div className='randomPlanet-container'>
        ...
        <div className='planetInfo-container'>
          ...
          <div className='planet-info'>
            <p className='planetInfo-text'>Population <span>{population}</span></p>
            <p className='planetInfo-text'>Rotation Period <span>{rotationPeriod}</span> </p>
            <p className='planetInfo-text'>Diameter <span>{diameter}</span></p>
          </div>
        </div>
      </div>
    );
